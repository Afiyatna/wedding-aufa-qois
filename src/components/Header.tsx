      ...
      <div className="container mx-auto px-4 py-4">
        <div className="flex items-center justify-between">
          ...
          <div className="text-2xl font-serif">
            <span className={`${isDark ? 'text-rose-300' : 'text-rose-600'}`}>A</span>
            <span className={`mx-2 ${isDark ? 'text-gray-300' : 'text-gray-600'}`}>&</span>
            <span className={`${isDark ? 'text-rose-300' : 'text-rose-600'}`}>Q</span>
          </div>
          ...
          <nav className="hidden md:flex items-center space-x-8">
            {menuItems.map((item) => (
              <button
                ...
                className={`transition-colors hover:text-rose-500 ${isDark ? 'text-gray-300' : 'text-gray-700'
                  }`}
              >
                {item.label}
              </button>
        ...
        {/* Mobile Navigation */}
        {isMenuOpen && (
          <nav className={`md:hidden mt-4 py-4 border-t ${isDark ? 'border-gray-700' : 'border-gray-200'
            }`}>
            <div className="flex flex-col space-y-4">
              {menuItems.map((item) => (
                <button
                  key={item.label}
                  onClick={() => scrollToSection(item.href)}
                  className={`text-left transition-colors hover:text-rose-500 ${isDark ? 'text-gray-300' : 'text-gray-700'
                    }`}
                >
                  {item.label}
                </button>
