  ...
  return (
    <section 
      ...
      className={`min-h-screen flex items-center justify-center relative overflow-hidden ${
        isDark ? 'bg-gray-900' : 'bg-gradient-to-br from-rose-50 via-pink-25 to-rose-100'
      }`}
      style={{
        backgroundImage: isDark ? 'none' : `url("data:image/svg+xml,%3Csvg width='60' height='60' viewBox='0 0 60 60' xmlns='http://www.w3.org/2000/svg'%3E%3Cg fill='none' fill-rule='evenodd'%3E%3Cg fill='%23f9a8d4' fill-opacity='0.1'%3E%3Ccircle cx='30' cy='30' r='2'/%3E%3C/g%3E%3C/g%3E%3C/svg%3E")`,
      }}
    >
      ...
      <div className="container mx-auto px-4 text-center relative z-10">
        <div className={`transition-all duration-1000 ${
          ...
          <p className={`text-lg md:text-xl mb-8 ${
            isDark ? 'text-gray-300' : 'text-rose-600'
          }`}>
            The Wedding of
          </p>
          ...
          <div className="flex items-center justify-center mb-8">
            <div className={`h-px w-20 ${isDark ? 'bg-gray-600' : 'bg-rose-300'}`}></div>
            <Heart 
              className={`mx-6 animate-pulse ${isDark ? 'text-rose-400' : 'text-rose-500'}`} 
              size={32} 
              fill="currentColor" 
            />
            <div className={`h-px w-20 ${isDark ? 'bg-gray-600' : 'bg-rose-300'}`}></div>
          ...
          <div className={`inline-flex items-center space-x-3 px-6 py-3 rounded-full ${
            isDark ? 'bg-gray-800 border border-gray-700' : 'bg-white/80 border border-rose-200'
          } backdrop-blur-sm`}>
          ...
          {/* Scroll Indicator */}
          <div className="mt-16">
            <div className={`animate-bounce ${isDark ? 'text-gray-400' : 'text-rose-400'}`}>
